...
var configSchema = exports_external.object({
  ...
  lvLidoVaultUtilAddress: exports_external.string(),
  rateApiEndpoint: exports_external.string(),
  chainSelectorName: exports_external.string(),
  gasLimit: exports_external.string(),
  isTestnet: exports_external.boolean()
});
var LVLidoVaultUtilAbi = [
  {
    name: "checkUpkeep",
    type: "function",
...
var checkUpkeepNeeded = (runtime2) => {
  ...
  runtime2.log("Checking if upkeep is needed (view call - no gas)...");
  runtime2.log(`lvLidoVaultUtilAddress: ${runtime2.config.lvLidoVaultUtilAddress}`);
  const callData = encodeFunctionData({
    abi: LVLidoVaultUtilAbi,
    functionName: "checkUpkeep",
    args: ["0x"]
  });
  runtime2.log(`Calling contract at: ${runtime2.config.lvLidoVaultUtilAddress}`);
  const result = evmClient.callContract(runtime2, {
    call: {
      to: hexToBase64(runtime2.config.lvLidoVaultUtilAddress),
      data: hexToBase64(callData)
    }
  }).result();
  runtime2.log(`result: ${result}`);
  const resultHex = bytesToHex(result.data);
  if (!result.data || result.data.length === 0 || resultHex === "0x") {
    runtime2.log("Contract returned empty data - no upkeep needed or contract not ready");
    return { upkeepNeeded: false, taskId: 0 };
  }
  runtime2.log(`resultHex: ${resultHex}`);
  const decoded = decodeFunctionResult({
    abi: LVLidoVaultUtilAbi,
    functionName: "checkUpkeep",
    data: resultHex
  });
  runtime2.log(`decoded: ${decoded}`);
  const upkeepNeeded = decoded[0];
  const performData = decoded[1];
  let taskId = 0;
  if (upkeepNeeded && performData && performData !== "0x") {
    const decodedTaskId = decodeAbiParameters([{ name: "taskId", type: "uint256" }], performData);
    taskId = Number(decodedTaskId[0]);
    runtime2.log(`Upkeep needed! Task ID: ${taskId}`);
  } else if (!upkeepNeeded) {
    runtime2.log("No upkeep needed");
  }
  return { upkeepNeeded, taskId };
};
var executePerformTask = (runtime2, taskId) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.chainSelectorName,
    isTestnet: runtime2.config.isTestnet
  ...
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  runtime2.log(`Executing performTask() for task ${taskId}...`);
  const reportData = encodeAbiParameters([{ name: "taskId", type: "uint256" }], [BigInt(taskId)]);
  ...
  const resp = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.lvLidoVaultUtilAddress,
...
var onCronTrigger = (runtime2, payload) => {
  runtime2.log("Cron trigger activated!");
  if (payload.scheduledExecutionTime) {
    const timestamp = Number(payload.scheduledExecutionTime);
    if (!isNaN(timestamp)) {
      runtime2.log(`Scheduled time: ${new Date(timestamp * 1000).toISOString()}`);
    }
  }
  const { upkeepNeeded, taskId } = checkUpkeepNeeded(runtime2);
  if (!upkeepNeeded) {
    runtime2.log("No action required - skipping (no gas spent)");
    return "no_action";
  }
  let txHash;
  txHash = executePerformTask(runtime2, taskId);
  runtime2.log(`Task ${taskId} executed successfully! TxHash: ${txHash}`);
  return txHash;
};
